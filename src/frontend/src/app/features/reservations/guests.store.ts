...
import { DEFAULT_GUEST_LIST_QUERY, hasActiveGuestFilters } from './guest-list-query';

/**
 * Misafir listesi signal store'u (`GET /guests`).
 *
 * Sorgu URL'den gelir; store son sorguyu, sonucu ve yukleme/hata durumunu
 * tutar. Silme **satir uzerinde** yurutulur ve aktif/gelecek rezervasyonu olan
 * misafirde sunucu **409** doner — bu anlamli bir mesaja cevrilir (hata cagirana
 * dondurulur ki ekran alan/satir hatasi olarak gosterebilsin).
 */
...
export class GuestsStore {
  ...
  private readonly _query = signal<GuestListQuery>(DEFAULT_GUEST_LIST_QUERY);
  ...
  readonly hasFilters = computed(() => hasActiveGuestFilters(this._query()));
  ...
  async load(query: GuestListQuery): Promise<void> {
  ...
  /** `DELETE /guests/{id}` — soft-delete; aktif/gelecek rezervasyon varsa 409. */
