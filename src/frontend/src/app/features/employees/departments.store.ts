...
/**
 * Departman signal store'u. Liste sayfalanmaz (sozlesme: duz dizi), bu yuzden
 * tek bir dizi + yukleme/hata durumu yeterlidir. Calisan listesi filtresi ve
 * calisan formunun departman secimi de bu store'u okur — veri kopyalanmaz.
 */
@Injectable({ providedIn: 'root' })
export class DepartmentsStore {
  private readonly api = inject(DepartmentsApi);

  private readonly _items = signal<readonly DepartmentResponse[]>([]);
  ...
  /** Filtre/secim listeleri icin ada gore siralanmis kopya. */
  readonly options = computed(() =>
    [...this._items()].sort((left, right) => left.name.localeCompare(right.name)),
  );

  /** `GET /departments`. `force` degilse ikinci kez ag istegi yapilmaz. */
  ...
  /** Duzenleme ekrani icin: sozlesmede tek kayit okuma ucu yok. */
  findById(id: string): DepartmentResponse | null {
    return this._items().find((item) => item.id === id) ?? null;
  }

  /**
   * `DELETE /departments/{id}` — **hard delete**; bagli calisan varsa backend
   * 409 doner ve kayit listede kalir.
   */
