...
/**
 * EmployeesPage — modul iskeleti.
 * Veri katmani (`employees.store.ts` + `core/api`) backend uc noktalari
 * hazir oldugunda eklenecektir; yerlesim, i18n ve RBAC koruması simdiden gecerlidir.
 */
@Component({
  selector: 'hc-employees',
  ...
  template: `
    <div class="space-y-6">
      ...
      <hc-empty-state
        titleKey="employees.empty.title"
        descriptionKey="employees.empty.description"
        eyebrowKey="common.underDevelopment"
      />

      <hc-card titleKey="common.underDevelopment" headingId="employees-pending">
